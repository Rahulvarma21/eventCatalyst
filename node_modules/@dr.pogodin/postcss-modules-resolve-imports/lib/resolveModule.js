...
const PRESERVE_SYMLINKS = argv['preserve-symlinks'] || String(process.env.NODE_PRESERVE_SYMLINKS) === '1';
...
function resolveModule(filepath, {
  ...
  const preserveSymlinks = resolvecfg.preserveSymlinks !== undefined ? Boolean(resolvecfg.preserveSymlinks) : PRESERVE_SYMLINKS;
  const file = applyAliases(filepath, resolvecfg.alias);
  const dirs = isNodeModule(file) ? (resolvecfg.modules || []).concat(nodeModulesPaths(cwd)) : (resolvecfg.modules || []).concat(cwd);
  for (let i = 0; i < dirs.length; ++i) {
    const directory = dirs[i];
    if (!isDirectory(directory)) continue;
    const abspath = resolve(directory, file);
    const result = resolveAsFile(abspath, resolvecfg.extensions) || resolveAsDir(abspath, resolvecfg.mainFile);
