...
function resolveDeps(ast, result) {
  ...
  ast.walkRules(moduleDeclaration, rule => {
    if (importDeclaration.exec(rule.selector)) {
      ...
      if (!(graph[absDependencyPath] && graph[absDependencyPath].mark === PERMANENT_MARK)) {
        ...
        const lazyResult = processor.process(css, Object.assign({}, result.opts, {
          from: absDependencyPath
        }));
  ...
  for (const token in moduleExports) for (const genericId in translations) moduleExports[token] = moduleExports[token].replace(genericId, translations[genericId]);
  ...
  const importNotes = comment({
    parent: rootTree,
    raws: {
      before: rootTree.nodes.length === 0 ? '' : '\n\n'
    },
