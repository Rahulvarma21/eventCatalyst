...
var regexExcessiveSpaces = /(^|\\+)?(\\[\dA-F]{1,6}) (?![\d A-Fa-f])/g;
var escape = function escape(string) {
  ...
  while (counter < string.length) {
    ...
    if (/[\t\n\u000B\f\r]/.test(character)) {
      ...
      value = "\\".concat(codePoint.toString(16).toUpperCase(), " ");
  ...
  if (/^-[\d-]/.test(output)) {
    output = "\\-".concat(output.slice(1));
  } else if (/\d/.test(firstChar)) {
    output = "\\3".concat(firstChar, " ").concat(output.slice(1));
  }

  // Remove spaces after `\HEX` escapes that are not followed by a hex digit,
  // since they’re redundant. Note that this is only possible if the escape
  // sequence isn’t preceded by an odd number of backslashes.
  output = output.replace(regexExcessiveSpaces, function (aa, bb, cc) {
    if (bb && bb.length % 2) {
...
function getLocalIdent(loaderContext, localIdentName, localName, options) {
  var relativeMatchResource = '';
  var context = options.context,
    hashPrefix = options.hashPrefix;
  var mdl = loaderContext._module,
    resourcePath = loaderContext.resourcePath;
  if (mdl && mdl.matchResource) {
    relativeMatchResource = "".concat(normalizePath(path.relative(context, mdl.matchResource)), "\0");
  }
  var relativeResourcePath = normalizePath(path.relative(context, resourcePath));

  // eslint-disable-next-line no-param-reassign
  options.content = "".concat(hashPrefix).concat(relativeMatchResource).concat(relativeResourcePath, "\0").concat(localName);
 ...
 */
module.exports = function getGenerateScopedName(context) {
  var template = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : '[hash:base64]';
  var hashPrefix = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : '';
  return function (localName, assetPath) {
    return escapeLocalident(getLocalIdent({
      resourcePath: assetPath
    }, template, unescape(localName), {
      context: context,
